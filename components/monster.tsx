...
export default function DMMonster ({ monster }: Props): React.JSX.Element {
  return (
    <div className="flex flex-row space-x-2 hover:border-sky-300 bg-slate-100 border-solid border-2 p-2 rounded w-full">
       {(monster.image != null) ? <img className="object-contain w-1/4" src={`https://www.dnd5eapi.co${monster.image}`} /> : null}
      <div className="flex flex-col w-full">
        ...
        {(monster.desc != null) ? <ReactMarkdown className="prose max-w-none" remarkPlugins={[remarkGfm]}>{`${monster.desc}`}</ReactMarkdown> : null}
...
function BaseRow ({ title, value }: { title: string, value: string | number }): React.JSX.Element {
  return (
